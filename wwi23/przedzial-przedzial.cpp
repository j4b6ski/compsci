...
void push(int akt)
{
    // 1. zaktualizowac "maks" uzywajac wartosci w "lazy"
    maks[akt] += lazy[akt];
...
// implementacja analogiczna do punkt-przedzial, ale dodajemy do lazy, zamiast do maks
// zawsze wolamy push na poczatku, bo musimy miec aktualne wartosci w tablicy "maks"
...
void add(int zap_pocz, int zap_kon, int wartosc, int akt=1, int akt_pocz=0, int akt_kon=n_max-1)
{
    ...
    else
    {
        ...
        maks[akt] = max(maks[2*akt], maks[2*akt + 1]); // aktualizujemy rodzica tak samo jak w punkt-przedzial
    }
}

// znajdz maksimum na przedziale [zap_pocz, zap_kon]
// implementacja tak samo jak w punkt-przedzial jedyna zmiana to 
// zawsze wolamy push na poczatku, zeby miec aktualne wartosci w tablicy "maks"
